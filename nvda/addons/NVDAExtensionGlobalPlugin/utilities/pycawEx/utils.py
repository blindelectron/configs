...
from .api.audioclient import ISimpleAudioVolume
from .api.audiopolicy import IAudioSessionControl2, IAudioSessionManager2
from .api.endpointvolume import IAudioEndpointVolume
from .api.mmdeviceapi import IMMDeviceEnumerator, IMMEndpoint
...
class AudioUtilities:
    ...
    def GetAudioSessionManager(audioDeviceID=None):
        speakers = AudioUtilities.GetSpeakers(audioDeviceID)
        if speakers is None:
            return None
        # win7+ only
    ...
    def GetAllSessions(audioDeviceID=None):
        audio_sessions = []
        mgr = AudioUtilities.GetAudioSessionManager(audioDeviceID)
        if mgr is None:
