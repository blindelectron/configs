...
DECTALK_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "dt460.dll"))
OWN_AUDIO_DEVICE = 1
...
class SynthDriver(synthDriverHandler.SynthDriver):
	name = "dectalk3"
	description = _("dectalk 4.60")  # NOQA: F821
	language = None
	supportedSettings = (
	...
	_voices = OrderedDict()
	_voices["Paul"] = {"name": "Perfect Paul", "pitch": 122, "inflection": 100, "volume": 81}
	_voices["Betty"] = {"name": "Beautiful Betty", "pitch": 208, "inflection": 240, "volume": 75}
	_voices["Harry"] = {"name": "Huge Harry", "pitch": 89, "inflection": 80, "volume": 76}
	_voices["Frank"] = {"name": "Frail Frank", "pitch": 155, "inflection": 90, "volume": 73}
	_voices["Dennis"] = {"name": "Doctor Dennis", "pitch": 110, "inflection": 135, "volume": 70}
	_voices["Kit"] = {"name": "Kit The Kid", "pitch": 296, "inflection": 180, "volume": 68}
	_voices["Ursula"] = {"name": "Uppity Ursula", "pitch": 240, "inflection": 135, "volume": 68}
	_voices["Rita"] = {"name": "Rough Rita", "pitch": 106, "inflection": 80, "volume": 76}
	_voices["Wendy"] = {"name": "Whispering Wendy", "pitch": 200, "inflection": 175, "volume": 70}
	minInflection = 0
	maxInflection = 250
	minPitch = 50
	maxPitch = 350
	...
	def __init__(self):
		...
		self.dt_rate = 180
		self.maxVolume = self._voices[self._voice]["volume"]
		self.dt_volume = self.maxVolume
		...
		dectalk.TextToSpeechStartup(self._messageWindow, byref(self.handle), 0, DO_NOT_USE_AUDIO_DEVICE)
		caps = TTS_CAPS_T()
...
#				item = item.replace("[:", " ")
	...
	def _set_voice(self, val):
		...
		self.maxVolume = self._voices[val]["volume"]
		self.dt_volume = self.maxVolume
		self._voice = val
		dectalk.TextToSpeechSpeak(self.handle, f"[:name {val[0].lower()}]".encode("latin-1"), 1)

	def _get_volume(self):
		return self._paramToPercent(self.dt_volume, self.minVolume, self.maxVolume)

	def _set_volume(self, volume):
		val = self._percentToParam(volume, self.minVolume, self.maxVolume)
		self.dt_volume = val
